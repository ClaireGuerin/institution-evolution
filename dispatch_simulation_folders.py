...
parser.add_argument('action', type=str, help='a string to indicate which action is required of the program: \
	"l" creates a list; \
	"f" creates a folder; \
	"fo" only creates a list of existant subfolders.')
...
### ACTION ###
if not any([i in args.action for i in ["f","l","o"]]):
	raise NameError(args.action + " is not a valid action argument. Use l, f or both (with o)")
else:
	if "l" in args.action:
		### CREATE LIST ###
